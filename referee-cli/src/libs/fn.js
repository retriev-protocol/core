...
const retrievefile = (provider, data_uri) => {
  return new Promise(async (response) => {
    try {
      if (
        data_uri.indexOf("ipfs://") !== -1 &&
        data_uri !== "ipfs://undefined"
      ) {
        ...
        try {
          ...
          console.log("File retrieved correctly.");
          console.log("File size is:", file.data.length);
          response(true);
        } catch (e) {
        ...
        console.log("Can't retrieve file, data_uri is not valid:", data_uri);
  ...
  try {
    ...
    if (appealsProcessed.indexOf(index.toString()) === -1) {
      ...
      if (round.toString() !== "99") {
        ...
        if (
          leader.toUpperCase() === wallet.address.toUpperCase() &&
          deal.owner !== "0x0000000000000000000000000000000000000000"
        ) {
          ...
          const retrieved = await retrievefile(
            provider.endpoint,
            deal.data_uri
          );
          ...
          if (retrieved === false) {
            console.log(
              "Slashing provider on-chain for appeal " + index + ".."
            );
            try {
              const slash = await contract.processAppeal(
                appeal.deal_index,
                [],
                []
              );
          ...
          } else if (retrieved === "INVALID_URI") {
            conosole.log("Original URI is not valid, can't do nothing.");
          } else {
          ...
          console.log(
            ...
            "waiting for " +
              halt_time +
              " seconds before try to retrieve file.."
          );
            ...
            try {
              if (appealsProcessed.indexOf(index.toString()) === -1) {
                ...
                if (retrieved === false) {
                  ...
                  // Run double check
                  const verified = await contract.verifyRefereeSignature(
                    signature,
                    index,
                    wallet.address
                  );
                  if (verified) {
                    console.log(
                      "Signature verified correctly, broadcasting to other referees.."
                    );
                    const message = JSON.stringify({
                      signature: signature,
                      appeal: index.toString(),
                    ...
                    console.log(
                      "Something goes wrong with signature, please check your instance."
                    );
                  }
                  // Repeating process
                  console.log(
                    "Waiting for 20s, then processing again appeal.."
                  );
                  setTimeout(function () {
                    processappeal(node, index);
                  }, 20000);
              ...
              console.log("Error while processing appeal..");
              console.log("--");
              console.log(e.message);
            }
          }, halt_time);
        ...
        try {
          await axios.post(
            "http://localhost:5001/api/v0/pin/rm?arg=" +
              deal.data_uri.replace("ipfs://", "/ipfs/")
          );
        } catch (e) {}
        appealsProcessed.push(index.toString());
        CONCURRENT_APPEALS--;
...
const parseslash = async (node, raw) => {
  try {
    ...
    const verified = (
      await node.verify(message.message, message.signature)
    ).toUpperCase();
    if (global["authorized"].indexOf(verified) !== -1) {
      ...
      if (leader.toUpperCase() !== wallet.address.toUpperCase()) {
        ...
        console.log(
          "[SLASH] Received slash message for appeal #" + slash.appeal
        );
        ...
        if (slash.received === true) {
          try {
            ...
            const provider = await contract.providers(ownerOf);
            console.log(
              "Retrieving again the file to check if leader is not corrupted.."
            );
            ...
            if (retrieved) {
              ...
              node.log(
                "ERROR_RETRIEVE_LEADER_" + appeal.deal_index.toString(),
                deal.data_uri
              );
          ...
          if (verified.toUpperCase() === leader.toUpperCase()) {
            console.log(
              "Received a slash message from leader, don't need to do nothing.."
            );
            appealsProcessed.push(slash.appeal.toString());
          } else {
            if (slash.signature !== undefined) {
              ...
              for (let f in processed[slash.appeal][round].signatures) {
                console.log(
                  processed[slash.appeal][round].signatures[f].signature,
                  slash.signature
                );
                if (
                  processed[slash.appeal][round].signatures[f].signature ===
            ...
            if (retrieved) {
              ...
              console.log(
                "File wasn't retrieved correctly, adding signature and try slash!"
              );
              ...
              if (verified) {
                ...
                for (let j in processed[slash.appeal][round].signatures) {
                  if (
                    unique.indexOf(
                      processed[slash.appeal][round].signatures[
                        j
                      ].referee.toLowerCase()
                    ) === -1
                  ) {
                    ...
                    try {
                      const verified = await contract.verifyRefereeSignature(
                        processed[slash.appeal][round].signatures[j].signature,
                        slash.appeal,
                        processed[slash.appeal][round].signatures[
                          j
                        ].referee.toLowerCase()
                      ...
                      if (verified) {
                        referees_addresses.push(
                          processed[slash.appeal][round].signatures[
                            j
                          ].referee.toLowerCase()
                        );
                        referees_signatures.push(
                          processed[slash.appeal][round].signatures[j].signature
                        );
                      }
                    } catch (e) {
                      console.log(
                        "Can't verify signature from:",
                        processed[slash.appeal][round].signatures[
                          j
                        ].referee.toLowerCase()
                      );
                ...
                if (parsedSignaturesCount >= threshold) {
                  ...
                  console.log(
                    "Slashing provider because leader didn't and collected enough signatures."
                  );
                  try {
                    const slashTransaction = await contract.processAppeal(
                      appeal.deal_index,
                      referees_addresses,
                      referees_signatures
                    );
                    console.log(
                      "Pending transaction at: " + slashTransaction.hash
                    );
                    ...
                    node.log(
                      "ERROR_SLASH_WITH_CONSENSUS_" +
                        appeal.deal_index.toString(),
                      e.message
                    );
                    console.log(
                      "Error while slashing provider, probably round processed yet.."
                    );
                  }
                } else {
...
async function bootstrap(node) {
  if (
    Object.keys(global["servers"]).length > 0 ||
    Object.keys(global["clients"]).length > 0
  ) {
    setuplisteners(node);
  } else {
