    ...
    for (let k in providersApi.data) {
      providers.push({
        ...
        max_collateral_multiplier:
          providersApi.data[k].strategy.max_collateral_multiplier,
...
const deals = async (node) => {
  ...
  try {
    let deals = [];
    const dealsApi = await axios.get(
      configs.api_url + "/deals/" + wallet.address
    );
  ...
  return new Promise(async (response) => {
    ...
    if (
      ...
      for (let j in providersApi.data) {
        // TODO: Handle multi provider case
        if (
          providersApi.data[j].address.toUpperCase() ===
      ...
      if (!found) {
        console.log(
          "Provider " + providers + " is invalid, please check the address."
        );
        process.exit();
      }
      ...
      if (argv.file !== undefined) {
        try {
          ...
          console.log("☁️  Uploading file to local node..");
          ...
          console.log("📦 File uploaded to IPFS:", uploaded.data);
          data_uri = "ipfs://" + uploaded.data.cid;
          if (data_uri === "ipfs://undefined") {
            throw new Error("Error while uploading file, retry.");
          }
      ...
      if (data_uri !== undefined) {
        ...
        if (
          ...
          parseInt(balance.toString()) >= value
        ) {
          try {
            let tx;
            if (parseInt(value) > 0) {
      ...
      console.log(
        "createdeal --file=<PATH_TO_LOCAL_FILE> --provider=<PROVIDER_ADDRESS> --duration=<DURATION>"
      );
      console.log("");
      console.log("Or use expert mode like:");
      console.log(
        "createdeal --dealuri=<DEAL_URI> --provider=<PROVIDER_ADDRESS> --duration=<DURATION> --value=<DEAL_VALUE> --collateral=<COLLATERAL_IN_WEI> --appealaddress=<APPEAL_ADDRESSES_SEPARATED_BY_COMMA>"
      );
    }
  });
}
...
function makeappeal(node) {
  return new Promise(async (response) => {
    ...
    if (deal_index !== undefined) {
      ...
      const can_create_appeal = await contract.canAddressAppeal(
        deal_index,
        wallet.address
      );
    ...
    if (argv.dealuri !== undefined || argv._[1] !== undefined) {
      try {
        ...
        if (argv._[1] !== undefined) {
          ...
          const timestamp_end =
            (parseInt(details.timestamp_start) + parseInt(details.duration)) *
            1000;
          if (
            details.data_uri.length > 0 &&
          ...
          for (let k in dealsApi.data) {
            if (
              dealsApi.data[k].data_uri.replace("ipfs://", "") ===
                argv.dealuri.replace("ipfs://", "") &&
              parseInt(dealsApi.data[k].timestamp_end) * 1000 >=
                new Date().getTime() &&
              !dealsApi.data[k].canceled
            ) {
        ...
        if (
          ...
          deal.provider !== undefined
        ) {
          const provider = await contract.providers(deal.provider);
          console.log(
            "Retrieving " + deal.data_uri + " from provider endpoint:",
            provider.endpoint
          );
