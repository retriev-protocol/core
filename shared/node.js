...
module.exports = class RetrievNode {
  ...
  constructor(node, port, daemon) {
    ...
    try {
      this.configs = JSON.parse(
        fs.readFileSync(this.nodePath + "/configs.json")
      );
    } catch (e) {
    ...
    if (this.configs === undefined) {
      ...
      if (
        process.env.WEB3_PROVIDER === undefined ||
        ...
        if (socket.handshake.query.identity !== undefined) {
          ...
          if (
            ...
            global["clients"][identity].on("disconnect", function () {
              ...
              console.log("Trying to reconnect to " + identity);
              rpp.connect(rpp.pairs[identity], identity);
            });
            // Relay messages if received by socket
            global["clients"][identity].on("message", async function (raw) {
              console.log(
                "[MESSAGE] New message from " + identity + ": " + raw
              );
  ...
  async bootstrap() {
    ...
    if (this.peers.length > 0) {
      for (let k in this.peers) {
        ...
        this.pairs[peer.identity] = peer.endpoint;
      }
    } else {
  ...
  async connect(node, identity) {
    ...
    socket.on("connect", () => {
      if (
        global["servers"][identity] === undefined &&
        identity !== this.configs.address
      ) {
  ...
  async parse(raw, protocol) {
    try {
      ...
      const verified = (
        await this.verify(message.message, message.signature)
      ).toUpperCase();
  ...
  async broadcast(message, protocol = "message") {
    ...
    const toBroadcast = JSON.stringify({
      message: message,
      signature: signature,
    });
  ...
  api() {
    ...
    app.post("/broadcast", async function (req, res) {
      ...
      if (verified.toUpperCase() === rpp.configs.address.toUpperCase()) {
        ...
        res.send({
          message: "Can't verify signer",
          signer: verified,
          owner: rpp.configs.address,
        });
      }
    });
    app.get("/ipfs/:hash", async function (req, res) {
      ...
      try {
        ...
        const file = await axios.post(
          "http://127.0.0.1:5001/api/v0/files/stat?arg=/ipfs/" + hash
        );
  ...
  async log(activity, details = "") {
    try {
      const signature = await this.sign(
        "Store " + this.configs.address + " activity."
      );
      await axios.post(this.configs.api_url + "/activity", {
        activity,
        details,
        signature,
  ...
  async contract() {
    ...
    const contract = new ethers.Contract(
      this.configs.contract_address,
      ABI.abi,
      wallet
    );
    provider.pollingInterval = parseInt(
  ...
  async getpeers() {
    let i = 0;
    let protocol = "https";
    if (argv.local !== undefined && argv.local === true) {
      protocol = "http";
    }
      ...
      try {
        ...
        if (provider !== "0x0000000000000000000000000000000000000000") {
          ...
          if (provider.toUpperCase() !== this.configs.address.toUpperCase()) {
            ...
            if (
              details.active &&
              details.endpoint.indexOf(protocol) !== -1 &&
              this.endpoints.indexOf(details.endpoint) === -1
            ) {
              ...
              console.log("This provider is not active or has no endpoint.");
            }
          }
        }
      } catch (e) {
    ...
    while (!ended) {
      try {
        ...
        if (referee !== "0x0000000000000000000000000000000000000000") {
          console.log("Checking referee: " + referee);
