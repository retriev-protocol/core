  ...
  for (let k in configs.referees) {
    if (mode === "leader") {
      if (configs.referees[k].address.toUpperCase() === leader.toUpperCase()) {
        ...
        if (
          parseInt(appeal.slashes.toString()) < parseInt(round.toString()) &&
          round.toString() !== "99"
        ) {
          try {
            const leaderWallet = new ethers.Wallet(
              configs.referees[k].key
            ).connect(provider);
            const leaderContract = new ethers.Contract(
              configs.contract_address,
              ABI.abi,
              leaderWallet
            );
            const slash = await leaderContract.processAppeal(
              appeal_index,
              [],
              []
            );
