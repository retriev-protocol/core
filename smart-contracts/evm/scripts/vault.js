...
async function main() {
  ...
  if (configs.providers !== undefined) {
    for (let k = 0; k < configs.providers.length; k++) {
      ...
      console.log(
        "Balance of " + configs.providers[k].address + " is:",
  ...
  if (configs.referees !== undefined) {
    ...
    for (let k = 0; k < configs.referees.length; k++) {
      ...
      console.log(
        "Balance of " + configs.referees[k].address + " is:",
        ethers.utils.formatEther(balance.toString())
      );
    }
  }
  ...
  console.log(
    "Balance of " + configs.owner_address + " is:",
    ethers.utils.formatEther(balanceowner.toString())
  );
  console.log("--");
  console.log("Asking for protocol balance..");
  const balanceprotocol = await contract.vault(
    configs.constructor_arguments[0]
  );
  console.log(
    "Balance of " + configs.constructor_arguments[0] + " is:",
    ethers.utils.formatEther(balanceprotocol.toString())
  );
}
