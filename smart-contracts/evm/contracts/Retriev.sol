...
contract Retriev is ERC721, Ownable, ReentrancyGuard {
    ...
    function redeemDeal(uint256 deal_index) external nonReentrant {
        ...
        require(
            pending_appeals[deals[deal_index].data_uri] == 0 ||
                (appeals[pending_appeals[deals[deal_index].data_uri]]
                    .request_timestamp + appeal_timeout) <
                block.timestamp,
    ...
    */
    function processAppeal(
        ...
        require(
            !checkDuplicate(_signatures),
            "processAppeal: Duplicate signatures"
        );
        require(deals[deal_index].timestamp_start > 0, "Deal is not active");
